...
import org.bls.gnt.resources.Lexicons;
import org.bls.gnt.resources.Texts;
...
public class GntService extends Application<GntConfiguration> {
	...
	public void run(final GntConfiguration configuration, final Environment environment) throws Exception {
		...
		GntServiceContext serviceContext = ConfigHelper.setupContext(configuration);
		...
		Lexicons exicons = new Lexicons(serviceContext);
		environment.jersey().register(exicons);
		
		Texts texts = new Texts(serviceContext);
		environment.jersey().register(texts);
		
		// Swagger API docs generation related
		environment.jersey().register(new ApiListingResource());
		environment.jersey().register(new SwaggerSerializers());
		BeanConfig config = new BeanConfig();
		config.setTitle("Gnt API Docs!");
