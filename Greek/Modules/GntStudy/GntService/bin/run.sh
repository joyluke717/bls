...
#
# 2. The default run.sh provide an simple but portable way of running Cible in background.
...
function server_start {
    ...
    OPTS="${JAVA_OPTS} \
    -cp ${CP} \
    com.huawei.cible.nexus.CibleNexusService \
    server ${CONFIG_FILE}"
    ...
    then
        ...
        ${JAVA_CMD} ${OPTS} &
        echo $! > ${PID_FILE}
        echo "Cible Nexus server started."
    fi
}
...
function server_stop {
    ...
    rm -fr ${PID_FILE}
    echo "Cible Nexus server stopped."
}
