...
public class DatabaseUtil {
	...
	public static BasicDataSource createMysqlDataSource(String host, int port, String user, String passwd,
			String poolSize) {
		// autoReconnect by default is false
		// TODO: queryTimeoutKillsConnection to true,
		// "?queryTimeoutKillsConnection=true"
		String url = String
				.format("jdbc:mysql://%s:%d/cible?connectTimeout=5000&socketTimeout=3000&characterEncoding=UTF-8"
						+ "&connectionCollation=utf8_general_ci", host, port);
		return createDataSource(MYSQL_JDBC_DRIVER, url, user, passwd, poolSize, MAX_WAIT_TIME_FOR_CONN_IN_MS);
	}

	public static BasicDataSource createMysqlDataSource(String host, int port, String user, String passwd,
			String poolSize, Map<String, String> connectionProperties) {
		// autoReconnect by default is false
		// TODO: queryTimeoutKillsConnection to true,
		// "?queryTimeoutKillsConnection=true"
		String url = String
	 ...
	 *            the url of the DB.
	 * @param user
	 *            the user name to connect to MySQL as.
	 * @param passwd
	 *            the password for the corresponding MySQL user.
	 * @param poolSize
	 *            the connection pool size string, in the format of
	 *            initialSize:maxActive:maxIdle:minIdle.
	 * @param maxWaitInMillis
	 *            the max wait time in milliseconds to get a connection from the
	 *            pool.
	...
	public static BasicDataSource createDataSource(String driverClassName, String url, String user, String passwd,
			String poolSize, int maxWaitInMillis, Map<String, String> connectionProperties) {
		...
		if (connectionProperties != null) {
			for (Map.Entry<String, String> entry : connectionProperties.entrySet()) {
				LOG.info(String.format("Add connection properties %s=%s", entry.getKey(), entry.getValue()));
